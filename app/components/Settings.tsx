...
export function Settings() {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={toggleSettings}>
      <div className="bg-zinc-900 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-6 sm:m-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-white">Settings</h2>
          <button onClick={toggleSettings} className="p-1.5 text-zinc-500 hover:text-white rounded-lg"><X size={18} /></button>
        </div>
        ...
        <button onClick={toggleSettings} className="w-full py-2.5 bg-white text-black rounded-xl text-sm font-medium mt-2 active:scale-[0.98] transition-transform">Done</button>
      </div>
    </div>
  );
}
